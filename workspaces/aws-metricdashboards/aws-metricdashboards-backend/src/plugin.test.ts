...
import { awsAutodashboardsPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [awsAutodashboardsPlugin],
    });

    await request(server).get('/api/aws-autodashboards/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/aws-autodashboards/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/aws-autodashboards/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/aws-autodashboards/todos/${createdTodoItem.id}`)
